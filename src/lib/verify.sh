...
run_verification() {
  ...
  check "01_AGENTS/limon" "[ -d '$OPENCLAW_WORKSPACE/01_AGENTS/limon' ]"
  check "04_SKILLS" "[ -d '$OPENCLAW_WORKSPACE/04_SKILLS/proposal_service' ]"
  check "SOUL Limón" "[ -f '$OPENCLAW_WORKSPACE/01_AGENTS/limon/SOUL.md' ]"
...
print_summary() {
  ...
  echo ""
  echo -e "    ${CYAN}Limón${NC}   - CEO y Coordinadora (gemini-2.5-pro)"
  ...
  if [ "$WANT_TELEGRAM" = true ]; then
    ...
    echo "       openclaw channels add telegram --token TOKEN --session agent:limon:main --allowed-users TU_ID"
  fi
